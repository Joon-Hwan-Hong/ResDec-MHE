"""
Fusion layer for combining multi-branch embeddings.

Concatenates pseudobulk, HGT, and cell transformer embeddings,
...
class FusionLayer(nn.Module):
    """
    Fuse representations from three branches.

    All cell types get 3 embeddings (pseudobulk + HGT + Set Transformer).
    SetTransformer embeddings are weighted by CellTypeSelector's soft attention.
    ...
    def __init__(
        ...
        # All types: concat 3 branches (cell_emb already weighted by selection)
    ...
    def forward(
        self,
        pseudobulk_emb: torch.Tensor,  # [B, 31, d_embed]
        hgt_emb: torch.Tensor,          # [B, 31, d_embed]
        cell_emb: torch.Tensor,         # [B, 31, d_embed] (already weighted by selection)
    ) -> torch.Tensor:
        """
        Fuse all three branches for all cell types.

        cell_emb is already scaled by CellTypeSelector's soft attention weights,
        so cell types with low importance contribute less to the fusion.

        Returns:
            fused: [B, 31, d_fused]
        """
        # Input dimension validation
        if pseudobulk_emb.dim() != 3:
            raise ValueError(f"Expected 3D input for pseudobulk_emb, got shape {pseudobulk_emb.shape}")
        if hgt_emb.dim() != 3:
        ...
        B, C, D = pseudobulk_emb.shape
        if C != self.n_cell_types:
            raise ValueError(f"Expected {self.n_cell_types} cell types, got {C}")
        if D != self.d_embed:
            raise ValueError(f"Expected d_embed={self.d_embed} for pseudobulk_emb, got {D}")
        if hgt_emb.shape[1] != self.n_cell_types:
            raise ValueError(f"Expected {self.n_cell_types} cell types for hgt_emb, got {hgt_emb.shape[1]}")
        if hgt_emb.shape[2] != self.d_embed:
            raise ValueError(f"Expected d_embed={self.d_embed} for hgt_emb, got {hgt_emb.shape[2]}")
        ...
        if cell_emb.shape[2] != self.d_embed:
