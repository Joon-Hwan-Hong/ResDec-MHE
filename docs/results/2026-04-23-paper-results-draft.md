...
To quantify uncertainty around the headline R², we computed a 95% bootstrap CI on the pooled composite prediction (resampling the 516 subjects 1000× without replacement of folds): R² = 0.4493, 95% CI [0.3731, 0.5070]. The CI is wide enough at this sample size that it overlaps with TabPFN-2.6's central estimate (0.3994), so the bootstrap CI alone does not separate ResDec-MHE from TabPFN-2.6 at the global-pooled-R² level; the improvement is demonstrated more decisively by the per-fold paired tests below, which condition on fold and exploit the same-subject pairing.

We extended the paired one-sided Wilcoxon signed-rank tests across all 5 seeds × 22 baselines (110 baseline-vs-seed comparisons total). Per-seed (n=5 folds), ResDec-MHE wins **5/5 fold pairs against every baseline at every seed**; the corresponding paired Wilcoxon W=15, p=0.0312 (the n=5 lower-bound) for almost every cell, with one cell (vs TabPFN-2.6 at seed 2000) at W=14, p=0.0625 due to a single fold whose Δ was within numerical noise (-0.002). Combining the 5 per-seed p-values per baseline via Stouffer's method gives one-sided combined p-values ranging from **2.9 × 10⁻⁵** (vs TabPFN-2.6) to **1.4 × 10⁻²** (vs the strongest non-TabPFN baseline, XGBoost [A+C+E]); all 22 are below 0.05. An across-seed sign test on the 5 seed-mean R²s gives p=0.0312 (5/5 wins) for every baseline. Full per-baseline test statistics are deposited at `outputs/redesign/interpretability/seed_variation_wilcoxon_all_baselines.json`.

A two-way crossed ANOVA decomposition of the 25 (5 seeds × 5 folds) per-fold R² values further isolates the source of variability: σ²_fold accounts for **97.6 %** of total variance (σ_fold = 0.094), while σ²_seed accounts for **0.2 %** (σ_seed = 0.004) and the seed×fold interaction + residual accounts for the remaining 2.3 %. Fold variance dominates by ~24×, confirming that the per-fold paired tests above (which condition on fold) are the appropriate inferential lens; seed variation contributes essentially zero noise. Decomposition in `outputs/redesign/interpretability/seed_variance_decomposition.json`.

Calibration was assessed using the per-subject TabPFN-2.6 standard deviation as a proxy for composite predictive uncertainty (the composite head produces a point estimate; no direct uncertainty channel is available). Empirical coverage at nominal levels [0.5, 0.68, 0.8, 0.95] was [0.43, 0.63, 0.75, 0.92] respectively, slightly under-covering at the lower nominal levels and within calibration at 95%. The proxy is conservative-leaning. Full bootstrap, paired-test, and coverage detail is reported in the supplementary `statistical_rigor.json`.
