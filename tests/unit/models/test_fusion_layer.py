...
class TestInitialization:
    ...
    def test_creates_projection_layer(self):
        """Should create projection from d_embed + d_cell_emb to d_fused."""
        from src.models.fusion.fusion_layer import FusionLayer

        layer = FusionLayer(d_embed=64, d_fused=128)

        # n_pma_seeds=1 by default, so d_cell_emb = 1 * 64 = 64
        assert layer.proj.in_features == 64 + 64
...
class TestGradientFlow:
    ...
    def test_gradients_flow_to_all_inputs(self):
        """Gradients should reach both input tensors."""
        from src.models.fusion.fusion_layer import FusionLayer
    ...
    def test_mismatched_d_embed_raises_error(self):
        """Inputs with wrong feature dimension should raise ValueError."""
        from src.models.fusion.fusion_layer import FusionLayer

        layer = FusionLayer(d_embed=64, d_fused=128, n_cell_types=N_CELL_TYPES)

        wrong_hgt = torch.randn(2, N_CELL_TYPES, 32)  # d_embed=32 != 64
        correct = torch.randn(2, N_CELL_TYPES, 64)

        with pytest.raises(ValueError):
            layer(wrong_hgt, correct)

    def test_both_branches_contribute_to_output(self):
        """Changing either branch input should change the output."""
        from src.models.fusion.fusion_layer import FusionLayer
        ...
        for i, modified in enumerate([
            (torch.randn_like(hgt), cell),
            (hgt, torch.randn_like(cell)),
        ]):
            alt = layer(*modified)
...
class TestValidation:
    ...
    def test_invalid_input_dimension_hgt(self):
        ...
        with pytest.raises(ValueError, match="Expected 3D input for hgt_emb"):
            layer(torch.randn(N_CELL_TYPES, 32), torch.randn(2, N_CELL_TYPES, 32))

        # 4D input for hgt
        with pytest.raises(ValueError, match="Expected 3D input for hgt_emb"):
            layer(torch.randn(2, 1, N_CELL_TYPES, 32), torch.randn(2, N_CELL_TYPES, 32))
    ...
    def test_invalid_input_dimension_cell(self):
        ...
        with pytest.raises(ValueError, match="Expected 3D input for cell_emb"):
            layer(torch.randn(2, N_CELL_TYPES, 32), torch.randn(N_CELL_TYPES, 32))
